...
def app_sidebar():
    st.sidebar.header('Credit Details')
    ...
    int_rate = st.sidebar.slider('Credit Interest Rate', 5, 40, 10, 1)
...
def app_body():
    title = '<p style="font-family:arial, sans-serif; color:Black; font-size: 40px;"><b> Welcome to DSSI Credit Assessment</b></p>'
    ...
    if st.session_state['input_features']:
        assessment = get_prediction(emp_length=st.session_state['input_features']['emp_length'],
                                    ...
                                    Credit_amnt=st.session_state['input_features']['Credit_amnt'])
